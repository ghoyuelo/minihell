...
void	redir_token_b(t_stack *stack)
{
	...
	if (tmp != NULL)
	{
		if (tmp->pipe.redir == 'b')
			redir_token_b_b(tmp);
		else if (tmp->pipe.redir == 'h')
		{
			fd = open_r_heredoc(tmp);
			tmp->prev->pipe.p[0] = fd;
		}
		...
		{
			if (tmp->prev->pipe.redir == 'f' || tmp->prev->pipe.redir == 'b'
				|| tmp->prev->pipe.redir == 'h' || tmp->prev->pipe.redir == 'a')
				tmp->pipe.p[0] = 0;
...
void	redir_token_b_b(t_stack node)
{
	t_stack	*countb;

	if (tmp->prev->pipe.redir == 'b' || tmp->prev->pipe.redir == 'h')
		tmp->prev->pipe.p[1] = tmp->pipe.p[1];
	else if (tmp->prev->pipe.redir != 'b' || tmp->prev->pipe.redir != 'h')
	...
	else
	{
		...
		if (countb->next == NULL)
			...
			tmp->prev->pipe.p[1] = countb->next->pipe.p[0];
	}
}
...
t_stack	*back_redir_count(t_stack *tmp)
{
	...
	tp = tmp;
	while (tp->next != NULL && (tp->next->pipe.redir == 'b'
		|| tp->next->pipe.redir == 'h' ))
		tp = tp->next;
	return (tp);
}
